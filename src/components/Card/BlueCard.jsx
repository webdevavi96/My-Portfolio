...
export default function BlueCard({ props }) {
  // Determine if hover effect should apply
  const hoverClass = !props.img && !props.link ? "hover:scale-105 transform transition duration-300" : ""

  return (
    <div
      className={`bg-gradient-to-tr from-blue-700 to-indigo-800 p-6 rounded-2xl shadow-xl flex flex-col ${hoverClass}`}
    >
      <h3 className="text-xl font-bold mb-1">{props.title}</h3>
